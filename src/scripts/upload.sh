...
if getcli > ./buildpulse-test-reporter; then
  ...
  echo ❌ Unable to send test results to BuildPulse. See details below.
  echo
  echo Downloading the BuildPulse test-reporter failed.
  echo
  echo We never want BuildPulse to make your builds unstable. Since we're having trouble downloading the BuildPulse test-reporter, we're skipping the BuildPulse analysis for this build.
  echo
  echo If you continue seeing this problem, please get in get in touch at https://buildpulse.io/contact so we can look into this issue.
